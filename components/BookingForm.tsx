...
// ТУТ ВАШІ ДАНІ ТЕЛЕГРАМ
const TELEGRAM_TOKEN = '7611388062:AAEqC6K9L-0_3F90D-M510X60X80'; // Приклад
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <button
        ...
        className="w-full bg-black text-white py-6 rounded-full font-bold uppercase tracking-[0.2em] text-[11px] hover:bg-neutral-800 transition-all disabled:opacity-50 shadow-xl hover:shadow-2xl translate-y-4"
      >
        {status === 'loading' ? 'Відправка...' : 'Підтвердити запис'}
      </button>

      {status === 'error' && (
        <p className="text-red-400 text-center text-[10px] uppercase tracking-widest mt-4">Виникла помилка. Спробуйте ще раз.</p>
      )}
    </form>
  );
};
